                        ...
                        if (inframe[0] in given_targets):
                            ...
                            if confidence >= 5:
                                ...
                                while not vehicle.mode.name=='GUIDED':
                                    ...
                                    print("waiting for mode change...")
                                print(vehicle.mode)
                                object_point = localization(vehicle.location.global_frame.lon, vehicle.location.global_frame.lat)
                                if (get_distance_metres(vehicle.location.global_frame, object_point) < 20):
                                    vehicle.simple_goto(object_point)
                                    x=0
                                    while(get_distance_metres(vehicle.location.global_frame, object_point) > 3):
                                        time.sleep(1)
                            ...
                            if len(inframe) == 2:
                                if (inframe[0] in given_targets or inframe[1] in given_targets):
                                    last_searched = nextwaypoint
                                    if confidence == 0:
                                        vehicle.mode = VehicleMode("LOITER")
                                        while not vehicle.mode.name=='LOITER':
