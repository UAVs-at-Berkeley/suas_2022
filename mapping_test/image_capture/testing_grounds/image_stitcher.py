...
def grabAndResize(col, row):
    pic = cv2.imread(f'{PATH_OF_SCRIPT}/({row}, {col}).png')
    return enforceResize(pic)

def stich_all(cols, rows):
    imgArr = [[grabAndResize(j, i) for j in range(cols)] for i in range(rows)]
    i = 0
    while os.path.exists(f'{PATH_OF_SCRIPT}/result{str(i)}.png'):
        i += 1
    cv2.imwrite(f'{PATH_OF_SCRIPT}/result{str(i)}.png', stitchMatrix(imgArr))
